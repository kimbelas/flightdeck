...
| `POST /run` | Ask (P4-T4). **Not under a profile function** and **not an SSE reply** — both were wrong in the sketch. It spawns `claude.exe` with `CLAUDE_CONFIG_DIR` set, because every profile function forces `--dangerously-skip-permissions` and silently swallows `--permission-mode` (D47, F.9.3); it answers **202 `{runId}`** and the records arrive as `ask` frames on `GET /stream` (D48), so the deck keeps exactly one live feed and a run survives a reload. Always `--max-budget-usd` and `--max-turns` (SEC-PROC-4); one run at a time, `busy` is 409. |
| `GET /quota` | **not built, and not planned** (P4-T3). The `quota` frame has carried both subscriptions' last-known `Quota` since P2-T3, replayed on connect, for the three reasons recorded there — no second polling loop, no `/status` paths in the DOM, and only a replayed frame fills the header on an idle machine. The recommendation is not a route either: `recommendRouting` is pure, in `contracts/`, and applied by the deck and by `flightdeck-core status` from the summary they already hold (D46). |
...
| `GET /analytics/spend` | P7-T3. Cost per week (eight, oldest first), subscription and slug folder, from `cost-state` as Claude Code computed it (D5). No parameters — the window is core's. Answers from the spend ledger's own tables, which a background walk fills with its own byte cursors (D61). |
