...
describe('ProjectsViewModel', () => {
  it('is empty before anything is imported, and says why rather than apologising', () => {
    const model = new ProjectsViewModel({ projects: [], refusal: undefined });
  ...
  it('gives every project a key, a name and its path', () => {
    const [line] = new ProjectsViewModel({ projects: [APP_NEXT], refusal: undefined }).lines;
  ...
  it('keys two spellings of one folder the same, so a re-import cannot duplicate a row', () => {
    ...
    const keys = new ProjectsViewModel({
      projects: [APP_NEXT, shouted],
      refusal: undefined,
    }).lines.map((l) => l.key);
  ...
  it('has nothing to say when the last import was taken', () => {
    expect(
      new ProjectsViewModel({ projects: [APP_NEXT], refusal: undefined }).problem,
    ).toBeUndefined();
  });
  ...
  it('turns every refusal core can send into a sentence', () => {
    ...
    for (const refusal of IMPORT_REFUSALS) {
      const problem = new ProjectsViewModel({ projects: [], refusal }).problem ?? '';
  ...
  it('says what to do about the two that are worth knowing', () => {
    expect(new ProjectsViewModel({ projects: [], refusal: 'missing' }).problem).toContain(
      'no folder',
    );
    expect(new ProjectsViewModel({ projects: [], refusal: 'config_directory' }).problem).toContain(
      'config directory',
    );
  });
});
...
describe('ProjectsViewModel — stack and git (P3-T2)', () => {
  ...
  it('carries the stack in the order core detected it', () => {
    const model = new ProjectsViewModel({
      projects: [APP_NEXT],
      refusal: undefined,
      statuses: reading(undefined, ['Next.js', 'Node']),
    });
  ...
  it('turns the counts into a phrase, in the order somebody acts on them', () => {
    const model = new ProjectsViewModel({
      projects: [APP_NEXT],
      refusal: undefined,
      statuses: reading({ conflicts: 2, dirty: 3, ahead: 1, behind: 4 }),
    });
  ...
  it('shows no branch for a detached HEAD but still says what is outstanding', () => {
    const model = new ProjectsViewModel({
      projects: [APP_NEXT],
      refusal: undefined,
      statuses: reading({ branch: undefined, dirty: 2 }),
    });
  ...
  it('surfaces the in-progress state, because it changes what every other count means', () => {
    ...
    const model = new ProjectsViewModel({
      projects: [APP_NEXT],
      refusal: undefined,
      statuses: reading({ progress: 'rebasing' }),
    });
