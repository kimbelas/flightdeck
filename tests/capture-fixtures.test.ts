...
describe('capture-fixtures scrub', () => {
  ...
  // P4-T4: `bypassPermissions` is 17 characters, so the length rule scrubbed the one field the Ask
  // fixture exists to assert on — whether the run was sandboxed. `permission_mode` was already
  // vocabulary from the hook payloads; the stream-json spells the same enum in camelCase.
  it('keeps both spellings of the permission mode enum readable', () => {
    const camel = { permissionMode: 'bypassPermissions' };
    expect(scrubValue(camel)).toEqual(camel);
    expect(scrubValue({ permission_mode: 'acceptEdits' })).toEqual({
