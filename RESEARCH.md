...
| Shell routing — bash | **Deleted in P4-T0** (D20). `~\.bashrc` had a second, diverging set — `claude-365` pinned `--model claude-fable-5` where PowerShell relies on `settings.json` = `claude-fable-5-1[1m]`, plus `claude-365-1m`, `claude-isg-1m`, `claude-isg-bg`, and an interactive launcher menu on every new shell that called them. Two sources of truth disagreed; there is now one. The `claude` alias remains as a signpost. |
