...
- **Never write to anything under `~/.claude*` without showing the diff first and backing the
  ...
  `npm run connect` / `npm run disconnect` (P1-T11) and the `?` sheet's keyboard helper (P5a-T7)
  are the only sanctioned writers: the plan is always computed and shown first — a dry run by
  default for the CLI, a diff you have to open for the helper — every file is backed up
  `*.bak-<timestamp>` before an atomic rename, and both have an exact inverse that restores byte
  for byte. Re-print JSON in the file's OWN line endings —
